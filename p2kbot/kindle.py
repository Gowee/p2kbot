...
async def push_file(sender_email, recipient_email, file_path, file_name=None):
    ...
    message[
        "Message-ID"
    ] = f"<p2kbot-{simple_sha1(sender_email + file_path)}@{EMAIL_DOMAIN}>"
    # TODO: make text meaningful
    message["Subject"] = "Push to Kindle"
    message.attach(
        MIMEText(
            "The email is sent by P2KBot to push books to Kindle.", _charset="utf-8"
        )
    )
    ...
    try:
        r = await send_email(
            message, sender_email, recipient_email, hostname=hostnames[0]
        )
        logger.debug(
        ...
        raise EmailDeliveryFailure(
            f"Failed to send email to kindle due to error {e.__class__.__name__}: {e}"
        ) from e
