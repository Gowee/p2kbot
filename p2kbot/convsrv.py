...
async def handle_convert(request):
    ...
    if not input_file_path:
        return web.json_response(
            {"success": False, "reason": "Invalid file path."}, status=400
        )
    input_file_path = os.path.realpath(input_file_path)
    stem, ext = os.path.splitext(input_file_path)
    if not ext:
        return web.json_response(
            {"success": False, "reason": "Invalid file extension."}, status=400
        )
    ...
    if not os.path.exists(output_file_path):
        ...
        plumber = Plumber(
            input_file_path, output_file_path, plumber_logger, plumber_logger
        )
...
app = web.Application()
app.add_routes((web.get("/cwd", handle_cwd), web.get("/convert", handle_convert)))
